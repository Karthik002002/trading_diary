...
const Charts: React.FC = () => {
	...
	const { data: heatmapData, isLoading: heatmapLoading } =
		useTradeHeatmap(filters);
	const { data: winlossRatio, isLoading: isWinlossRatioLoading } =
		useWinLossPieChart(filters);
	const { data: treemapData, isLoading: treemapLoading } =
		useEmotionalTreemap(filters);
	...
	const aggregateData = [
		...
		{
			x: "Avg Returns",
			y: Math.abs(totalReturns / (timeseriesData.length || 1)),
		},
				...
				<div className="sticky top-2 z-1000 grid grid-cols-16 sm:grid-cols-8 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12 gap-4 p-4 rounded-lg items-end bg-slate-800 mb-6">
					<div className="col-span-2">
						<Select
							...
							value={
								filters.portfolio_id ? Number(filters.portfolio_id) : undefined
							}
					...
					<div className="col-span-2">
						<Select
							...
							value={
								filters.strategy_id ? Number(filters.strategy_id) : undefined
							}
