...
const Sidebar = ({ isCollapsed, toggleSidebar }: SidebarProps) => {
  ...
  // Global hotkey: Cmd/Ctrl + I -> toggle sidebar
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      // Don't trigger when typing in input/textarea/select or content editable
      ...
      const isModifierPressed = e.ctrlKey || e.metaKey; // ctrl on windows/linux, meta (cmd) on mac
      const keyIsI = e.key && e.key.toLowerCase() === "i";
      if (isModifierPressed && keyIsI) {
        e.preventDefault();
        toggleSidebar();
      }
    }
    ...
    >
      ...
      <nav className="flex-1 p-3 space-y-2">
        ...
        <SidebarNavItem
          ...
          isCollapsed={isCollapsed}
          isActive={false}
          // provide a customIcon prop that uses the customIcon (tabler xlsx)
          customIcon={<FileExcelOutlined style={{ color: activeLocation.pathname === "/excel-viewer"?"#7b61ff":"white" }} />}
        />
      </nav>
