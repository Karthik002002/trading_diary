...
retriever     = TradeRetriever(openai_client, chroma_path)
...
def ask(request: AskRequest):
    ...
    if retriever.collection.count() == 0:
        raise HTTPException(
            status_code=400,
            detail="No trades indexed yet. Run: python ingest/ingest.py"
        )
...
def trigger_ingest():
    ...
    try:
        ...
        from ingest.ingest import fetch_trades_from_mongo, ingest_trades
