...
from api.retriever import TradeRetriever
from api.answerer import TradeAnswerer
...
def run_test():
    ...
    # ── Step 2: Ingest into a test ChromaDB ─────────────────────────────────
    print("Step 2: Ingesting into test ChromaDB...")
    openai_client = OpenAI(api_key=api_key)

    # Use a separate test path so we don't pollute the real DB
    test_chroma_path = "./chroma_test"
    chroma_client    = chromadb.PersistentClient(path=test_chroma_path)
