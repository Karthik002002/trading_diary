...
export interface IForexTrade extends Document {
...
const ForexTradeSchema: Schema = new Schema(
    {
...
ForexTradeSchema.pre("save", function () {
    const trade = this as unknown as IForexTrade;
    ...
    if (entry_price == null || exit_price == null || quantity == null || !type) {
        return;
    }
    ...
    if (trade.exits && trade.exits.length > 0) {
        ...
        for (const exit of trade.exits) {
            if (type === "buy") {
                grossProfit += (exit.price - entry_price) * exit.quantity * contract_size;
            } else if (type === "sell") {
                grossProfit += (entry_price - exit.price) * exit.quantity * contract_size;
            }
        ...
        if (remainingQty > 0 && exit_price != null) {
            if (type === "buy") {
                grossProfit += (exit_price - entry_price) * remainingQty * contract_size;
            } else if (type === "sell") {
                grossProfit += (entry_price - exit_price) * remainingQty * contract_size;
            }
        }
    } else {
        if (type === "buy") {
            grossProfit = (exit_price - entry_price) * quantity * contract_size;
        } else if (type === "sell") {
            grossProfit = (entry_price - exit_price) * quantity * contract_size;
        }
    }
    ...
    if (stop_loss != null && take_profit != null) {
        ...
        trade.planned_rr = plannedRisk > 0 ? plannedReward / plannedRisk : 0;
    } else {
        trade.planned_rr = 0;
    }
    ...
    if (stop_loss != null) {
        ...
        trade.actual_rr = actualRisk > 0 ? actualReward / actualRisk : 0;
    } else {
        trade.actual_rr = 0;
    }
    ...
    // Returns %
    // ------------------------
    // In Forex, returns % is often relative to margin or balance, 
    // but here we use relative to notional value for consistency.
    const totalInvestment = entry_price * quantity * contract_size;
    const returnPercent = totalInvestment > 0 ? (grossProfit / totalInvestment) * 100 : 0;
    trade.returns = Number(returnPercent.toFixed(2));
});

export default mongoose.model<IForexTrade>("ForexTrade", ForexTradeSchema);
