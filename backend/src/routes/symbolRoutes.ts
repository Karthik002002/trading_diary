...
import { createSymbolValidator, updateSymbolValidator } from '../validators/symbolValidators';
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const symbol = await Symbol.findOneAndDelete({ id: parseInt(req.params.id) });
    if (!symbol) return res.status(404).json({ message: 'Symbol not found' });
